...
{
    fprintf (stderr, "Usage: %s [{-h HUBNUM | -b BUSNUM -d DEVNUM}] \\\n"
...
static void hub_port_status (usb_dev_handle *uh, int nport)
{
    ...
    for (i = 0; i < nport; i++) {
        ...
        int ret = usb_control_msg (uh, USB_ENDPOINT_IN | USB_TYPE_CLASS | USB_RECIP_OTHER, USB_REQ_GET_STATUS, 0, i + 1, buf, USB_STATUS_SIZE, CTRL_TIMEOUT);
        if (ret < 0) {
            fprintf (stderr, "cannot read port %d status, %s (%d)\n", i + 1, strerror(errno), errno);
            break;
        }
        ...
        for (dev = bus->devices; dev; dev = dev->next) {
            ...
            if (uh != NULL)
            {
                ...
                if ((len = usb_control_msg (uh, USB_DIR_IN | USB_RT_HUB, USB_REQ_GET_DESCRIPTOR, USB_DT_HUB << 8, 0, buf, sizeof (buf), CTRL_TIMEOUT)) > sizeof (struct usb_hub_descriptor))
                {
                    ...
                    if (print) {
                        printf ("Hub #%d at %s:%03d\n", number_of_hubs_with_feature, bus->dirname, dev->devnum);
                    }
...
int main (int argc, const char *argv[])
{
    ...
    if (uh == NULL) {
        ...
        if (usb_control_msg (uh, USB_RT_PORT, request, feature, index, NULL, 0, CTRL_TIMEOUT) < 0) {
